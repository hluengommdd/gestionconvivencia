...
const GestionEvidencias: React.FC = () => {
  ...
  const [evidencias, setEvidencias] = useState<Evidencia[]>(mockEvidencias);
  ...
  useEffect(() => {
    ...
    const loadEvidencias = async () => {
      ...
      const mapped = data.map((row: any): Evidencia => ({
        id: row.id,
      ...
      const signed = await Promise.all(mapped.map(async (ev) => {
        ...
        if (raw.includes('.supabase.co')) {
          const urlMatch = raw.match(/\/storage\/v1\/object\/sign\/(.+)$/);
          if (urlMatch) {
            path = urlMatch[1];
          }
        }
        ...
        const bucket = path.startsWith('evidencias-') 
          ? path.split('/')[0] 
          : 'evidencias-publicas';
        
        // Normalizar el path (quitar bucket si existe)
        const normalizedPath = path.startsWith('evidencias-') 
          ? path.split('/').slice(1).join('/') 
          : path;
        
        if (!normalizedPath) return ev;
        
        try {
          const { data: signedData } = await client
            .storage
            .from(bucket)
          ...
          if (supabase && expedienteTarget?.dbId) {
            ...
            if (userId) {
              ...
              if (archivoSeleccionado) {
                ...
                const path = `${expedienteTarget.dbId}/${Date.now()}-${archivoSeleccionado.name}`;
              ...
              if (!error && inserted?.id) {
                ...
                if (inserted.url_storage) {
                  const bucket = payload.esSensible ? 'evidencias-sensibles' : 'evidencias-publicas';
                  const raw = inserted.url_storage;
                  const parts = raw.split('/');
                  const b = raw.startsWith('evidencias-') ? parts[0] : bucket;
                  const p = raw.startsWith('evidencias-') ? parts.slice(1).join('/') : raw;
                  const { data: signedData } = await supabase.storage.from(b).createSignedUrl(p, 60 * 10);
